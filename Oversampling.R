...
-----#definicion  funcion ganancia para nuestro problema----
  ...
  ganancia = function( probs, clases ){
    ...
    for( i in 1:largo )
    {
      if( probs[ i, "BAJA+2"]  > ( 250/(8000/300))   ){ suma <- suma + if( clases[i]=="BAJA+2" ) { 7750 } else { -250 }  
      } ;
    }
...
for(  vmaxdepth  in  3:12 )
{
  ...
  for( s in  1:5 )
  {
    ...
    set.seed( semilla[s] )
    abril_inTraining <- createDataPartition( abril.over$clase, p = .70, list = FALSE)
    abril_dataset_training <- abril.over[ abril_inTraining,]
    abril_dataset_testing  <- abril.over[-abril_inTraining,]
