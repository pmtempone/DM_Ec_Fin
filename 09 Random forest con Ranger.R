...
archivo_entrada <- "/home/pablo/Documentos/TP_DM_Ec_Fin/abril_binario1.txt"
archivo_salida  <- "/home/pablo/Documentos/TP_DM_Ec_Fin/salida_ranger_umbral.txt"
  ...
  for(  vmin.node.size  in  c( 10000, 5000, 3000, 2000, 1000, 800, 700, 600, 500, 300, 200, 100, 50, 20) )
  {
    ...
    if( linea > lineas_archivo  )
    {
      for( s in  1:4 )
      {
        ...
        set.seed( semilla[s] )
        abril_inTraining <- createDataPartition( abril_b1$clase_binaria1, p = .70, list = FALSE)
        abril_dataset_train    <- abril_b1[ abril_inTraining,]
        abril_dataset_testing  <- abril_b1[-abril_inTraining,]
