...
saveRDS(abril_dataset,file="abril.rds")

abril <- readRDS("abril.rds")
...
table(abril$clase_binaria1)
levels(abril$clase_binaria1) <- c("POS","NEG")
abril$participa <- NULL
abril$edad_bins <- NULL
abril$foto_mes <- NULL
abril$mes <- NULL
set.seed(949)
...
set.seed(949)
mod0 <- train(clase_binaria1 ~ ., data = abril_dataset_training,
              method = "rf",
              metric = "ROC",
